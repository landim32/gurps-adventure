# NelsOwned — "O Pequeno Gigante da Canção"

**Jogador:** André · **Raça:** Anão (*GURPS Fantasy*, 30 pts) · **Arquétipo:** Trovador (*GURPS Fantasy*) · **Pontos:** 150/150

## Aparência

NelsOwned é um anão de baixa estatura, mas presença de palco colossal: veludos berrantes, barba
impecavelmente alinhada e uma voz potente e dramática, inspirada nos grandes cantores
românticos. Nada nele lembra os hinos de guerra anões tradicionais.

## História

Enquanto seus parentes cantam sobre batalhas e forjas, NelsOwned trocou tudo isso por baladas
melancólicas sobre amores impossíveis e corações partidos. Em combate, faz jus ao trocadilho do
próprio nome: humilha os inimigos com deboches melódicos fulminantes e notas agudas de pura
vergonha alheia, transformando o drama brega em uma arma tão ou mais letal que o seu grande
machado.

## Atributos

| Atributo | Valor | Custo |
|---|---|---|
| ST | 13 (11 base + 2 racial) | 10 |
| DX | 12 | 20 |
| IQ | 13 | 30 |
| HT | 12 | 20 |

**Total Atributos: 80 pontos**

Bônus racial de Anão: +2 ST (já somado acima), +2 Fadiga, RD +1 natural, Longevidade, escala de
Carga ampliada (2,5/5/10/15×ST) e -1 em Deslocamento por causa das pernas curtas — tudo incluído
no pacote racial de 30 pontos (ver Vantagens).

## Estatísticas Derivadas

- **Fadiga:** 15 (ST 13 + 2 racial)
- **Pontos de Vida:** 12
- **Dano Básico:** GDP 1D / Bal 2D-1
- **Velocidade Básica:** 6,0
- **Deslocamento:** 5 (6,0 de Vel. Básica − 1 por pernas curtas de Anão; carga "Nenhuma" não penaliza)
- **Carga (kg):** Nenhuma 13 · Leve 32,5 · Média 65 · Pesada 130 · Muito Pesada 195
- **Defesa Passiva/RD:** 1 (RD natural de Anão — não usa armadura, só os veludos de palco)
- **Esquiva:** 5 · **Aparar (Machado/Maça):** 7 · **Bloqueio:** —
- **Reação básica:** +2 (Voz Melodiosa e Carisma superam a Fanfarronice quando ele está falando/cantando)

## Vantagens e Desvantagens

| Nome | Custo |
|---|---|
| Raça: Anão (Fantasy) | +30 |
| Voz Melodiosa | +10 |
| Carisma +2 | +10 |
| Reflexos em Combate | +15 |
| Talento Musical +2 | +2 |
| Fanfarronice | -10 |
| Excesso de Confiança | -10 |
| Luxúria | -15 |
| Hábito Detestável (trocadilhos de mau gosto) | -5 |

**Vantagens: 67 pontos · Desvantagens: -40 pontos** (no limite do projeto)

Notas de arbitragem:
- **Fanfarronice** — adora provocar e humilhar quem estiver ao alcance da voz; teste de Vontade
  para evitar uma bravata grosseira quando sabe que não deveria.
- **Excesso de Confiança** — acredita que sua arte é sempre suficiente para vencer qualquer
  confronto; teste de IQ sempre que precisar agir com cautela.
- **Luxúria** — não resiste a um romance impossível; teste de Vontade (-5/-10 conforme a beleza
  do alvo) em qualquer contato com alguém atraente.
- **Hábito Detestável** — trocadilhos de mau gosto o tempo todo (a começar pelo próprio nome);
  -1 em testes de reação de quem tiver que aturá-lo.

## Peculiaridades

1. Solta um "Aaaah" dramático antes de qualquer fala séria.
2. Alinha a barba com cuidado antes de entrar em cena (ou em combate).
3. Termina frases tensas com uma rima involuntária.
4. Carrega um lenço de veludo só para "as lágrimas do público".
5. Exige ser chamado de "Pequeno Gigante da Canção".

**Peculiaridades: -5 pontos**

## Perícias

| Perícia | NH | Tipo | Custo | Categoria |
|---|---|---|---|---|
| Machado/Maça | 14 | DX/M | 6 | Combate |
| Trovador | 20 | IQ/M | 8 | Sociais |
| Canto | 18 | HT/F | 4 | Sociais |
| Instrumento Musical (Alaúde) | 16 | IQ/D | 6 | Sociais |
| Trato Social | 16 | IQ/F | 2 | Sociais |
| Sex-Appeal | 14 | HT/M | 2 | Sociais |
| Poesia | 14 | IQ/M | 4 | Sociais |
| Detecção de Mentiras | 13 | IQ/M | 2 | Ladinas |
| Manha | 13 | IQ/M | 2 | Ladinas |
| Lábia | 14 | IQ/M | 4 | Ladinas |
| Dança | 13 | DX/M | 4 | Físicas |
| Comércio | 14 | IQ/M | 4 | Profissionais |

**Total Perícias: 48 pontos**

Notas de cálculo:
- Anões têm Machado/Maça grátis em NH = DX; os 6 pontos pagos elevam de NH 12 para NH 14.
- Trovador e Canto somam o bônus permanente de +2 da Voz Melodiosa (Trovador soma também +2
  de Carisma, conforme o próprio texto da perícia). Instrumento Musical e Canto usam o Talento
  Musical (+2) como bônus ao atributo-base só para fins de cálculo do custo de aprendizado.

Em combate, o "ataque" mais temido de NelsOwned não está na ficha como perícia própria: é a
narrativa de usar Trovador/Lábia/Canto (NH 20/14/18) para debochar de um inimigo em plena
melodia, buscando gerar vergonha, distração ou um teste de reação/Vontade ruim antes de sacar o
machado — que ele leva como plano B.

## Equipamento

| Item | Dano | Tipo | Qtd | Peso (kg) |
|---|---|---|---|---|
| Machado grande (2 mãos) | 2D+2 | corte | 1 | 4,0 |
| Alaúde | - | - | 1 | 1,0 |
| Traje de veludo cênico | - | - | 1 | 2,0 |
| Cancioneiro de baladas | - | - | 1 | 0,3 |
| Rações (5 dias) | - | - | 1 | 1,25 |
| Moringa (1 litro) | - | - | 1 | 1,0 |
| Manta de viagem | - | - | 1 | 2,5 |
| Equip. básico pessoal | - | - | 1 | 0,5 |

**TOTAIS:** $418 · 12,55 kg (carga "Nenhuma" para ST 13 anã — sem penalidade em Deslocamento)

Sem armadura: ele confia nos veludos, na barba e na própria voz — a RD 1 natural de Anão é toda
a proteção que carrega.

## Resumo de Pontos

| Categoria | Pontos |
|---|---|
| Atributos | 80 |
| Vantagens | 67 |
| Desvantagens | -40 |
| Peculiaridades | -5 |
| Perícias | 48 |
| **Total** | **150** |
